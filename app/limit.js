...
.then(async () => {
    ...
    for (var i = 0; i < 100; i++) {
        await madridCentral.addResident(5000000000000000, "Rodri", "r@r", "passord", 666778899, 10, {from: accounts[i], gas: 5000000, gasPrice: '30000000'});
    }

    var residents = [];

    for (var i = 0; i < 100; i++) {
        residents[i] = await madridCentral.residents.call(accounts[i]);
    }

    for (var i = 0; i < 100; i++) {
         console.log("Resident " + i + " = ", residents[i]);
    }
